...
RSpec.describe "Donors", type: :request do
  ...
  describe "GET /donors" do
    it "checks the result of index query" do
      ...
      expect(@response[0]["donor_user"]).to have_key("age")
      expect(@response[0]["donor_user"]).to have_key("country")
      expect(@response[0]["donor_user"]).to have_key("address")
      expect(@response[0]["donor_user"]).to have_key("pan")
      expect(@response[0]["donor_user"]).to have_key("is_onboarded")
      expect(@response[0]["donor_user"]).to have_key("id_card")
  ...
  describe "POST /donors" do
    it "checks the creation of new donors" do
      ...
      expect(response).to have_http_status(200)
      @response=JSON.parse(response.body)
      expect(@response).to have_key("donor_reg_no")
      ...
      expect(@response["donor_user"]).to have_key("age")
      expect(@response["donor_user"]).to have_key("country")
      expect(@response["donor_user"]).to have_key("address")
      expect(@response["donor_user"]).to have_key("pan")
      expect(@response["donor_user"]).to have_key("is_onboarded")
      expect(@response["donor_user"]).to have_key("id_card")
    ...
    it "checks the validation of name field in donor" do
      ...
      @response=JSON.parse(response.body)
      expect(@response["status"]).to eq("Validation failed: Donor user name can't be blank")
    end
    it "checks the validation of phonenumber field in donor" do
      ...
      expect(Donor.count).to be(2)
      @response=JSON.parse(response.body)
      expect(@response["status"]).to eq("Validation failed: Donor user phonenumber has already been taken")
    end
  end
  describe "PUT /donors" do
    it "updates the donor details" do
      ...
      donor1.reload
      expect(donor1.donor_user.name).to eq("gourav")
      expect(donor1.donor_user.email).to eq("gourav@gmail.com")
    end
    it "updates the donor_user deatils using invalid mail id" do
      put api_v1_donor_path(donor1.id),headers:{"Authorization":"Bearer "+access_token.token},params:{"donor":{
        "donor_user_attributes": {
          "id":donor_user1.id,
            "phonenumber": "111111111191",
    ...
    @response=JSON.parse(response.body)
    expect(@response["donor_user.email"][0]).to eq("Invalid mail Id")
    end
    it "updates the donor_user deatils using duplicate phonenumber" do
      put api_v1_donor_path(donor1.id),headers:{"Authorization":"Bearer "+access_token.token},params:{"donor":{
        "donor_user_attributes": {
          "id":donor_user1.id,
            "phonenumber": donor_user2.phonenumber,
            "name": "gourav",
            "email":"dshjdjj@gmail.com",
            "guardian_name": "shjahjh",
            "country": "india",
            "gender": "male",
            "age": 21,
    ...
    expect(response).to have_http_status(422)
    @response=JSON.parse(response.body)
    expect(@response["donor_user.phonenumber"][0]).to eq("has already been taken")
    end
  end
  describe "PUT /subscription" do
    it "updates the donor subscription" do
      put api_v1_donor_path(donor1.id),headers:{"Authorization":"Bearer "+access_token.token},params:{"subscription_id":subscription2.id}
      expect(response).to have_http_status(200)
      @response=JSON.parse(response.body)
    ...
    describe "Checks the endpoints for family donors" do
      ...
      it "updates the family subscription through group member" do
        put api_v1_donor_path(donor2.id),headers:{"Authorization":"Bearer "+access_token.token},params:{"subscription_id":subscription2.id}
        expect(response).to have_http_status(422)
        @response=JSON.parse(response.body)
      ...
      it "validates the promotion of donor to area_representative as a group head" do
        put api_v1_donor_path(donor1.id),headers:{"Authorization":"Bearer "+access_token.token},params:{"donor":{"is_area_representative":true}}
        expect(response).to have_http_status(200)
        @response=JSON.parse(response.body)
  ...
  describe "PUT /deactivate" do
    it "deactivates the user" do
      put api_v1_donor_path(donor1.id),headers:{"Authorization":"Bearer "+access_token.token},params:{"status":false}
      expect(response).to have_http_status(200)
  ...
  describe "GET /show" do
    ...
    it "validates the promotion of group member to area_rep" do
      put api_v1_donor_path(donor2.id),headers:{"Authorization":"Bearer "+access_token.token},params:{"donor":{"is_area_representative":true}}
      expect(response).to have_http_status(422)
      @response=JSON.parse(response.body)
      expect(@response["is_area_representative"][0]).to eq("Group member can't be promoted")
    end
  ...
  describe "POST /representatives" do
    ...
    it "demote an area representative who isn't" do
      put demote_rep_api_v1_donor_path(donor2.id),headers:{"Authorization":"Bearer "+access_token.token},params:{"representative_id":donor3.id}
        expect(response).to have_http_status(200)
        @response=JSON.parse(response.body)
        expect(@response["status"]).to eq(false)
        expect(@response["message"]).to eq("Current donor is not area representative")
    end
  ...
  describe "GET /api/v1/donors/find" do
    it "find the donor via phonenumber" do
      get "/api/v1/donors/find",headers:{"Authorization":"Bearer "+access_token.token},params:{"field":"phonenumber","value":donor1.donor_user.phonenumber}
      expect(response).to have_http_status(200)
      @response=JSON.parse(response.body)
      expect(@response[0]["donor_id"]).to eq(donor1.id)
    end
  end
end
